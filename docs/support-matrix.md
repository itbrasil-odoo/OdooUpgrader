...
5. Use `--analyze-modules --analyze-modules-only` to preflight installed modules against local addons/OCA target branches.
6. When restoring dumps generated by newer PostgreSQL toolchains, set `--postgres-version` accordingly.
